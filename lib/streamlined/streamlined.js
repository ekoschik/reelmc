...
streamlined.startDaemon = function(pidfile) {
    if (!$streamlined_daemon) {
        if (fs.lstatSync(pidfile).isDirectory()) {
            pidfile = path.join(pidfile, path.basename(__filename) + '.pid');
        }
        console.log('starting service with pidfile ' + pidfile);
        $streamlined_daemon = daemonize.setup({
            main: __filename,
            pidfile: pidfile
        });
    }

    $streamlined_daemon.start();
};

streamlined.stopDaemon = function() {
    if ($streamlined_daemon) {
        $streamlined_daemon.stop();
        $streamlined_daemon = null;
    }
};
